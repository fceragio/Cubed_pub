...
/*   Updated: 2025/09/28 03:21:13 by federico         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static int	red_x_press_handling(t_program *program)
{
	program_close(program, SUCCESS);
	return (SUCCESS);
}

static int	visibility_handling(t_program *program)
{
	(void)program;
	return (SUCCESS);
}

static int	key_press_handling(int keysym, t_program *program)
{
	if (keysym == XK_Escape)
		program_close(program, SUCCESS);
	if (keysym == XK_w)
	...
	if (keysym == XK_m || keysym == XK_p || keysym == XK_space)
		bonus_press_handling(keysym, program);
	return (SUCCESS);
}

static int	key_release_handling(int keysym, t_program *program)
{
	if (keysym == XK_w)
...
void	hook_handlers(t_program *program)
{
	...
	mlx_hook(program->mlx_data.win, MotionNotify,
		PointerMotionMask, mouse_movement, program);
}
